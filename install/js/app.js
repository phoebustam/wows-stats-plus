...
app.controller('InstallCtrl', function ($scope, $http) {
	...
	$http({
		method:'GET',
		url: 'http://'+window.location.hostname+':'+window.location.port+':8080/api/env'
	...
	$scope.api.validatePath = function() {
		...
		$http({
			method:'POST',
			url: 'http://'+window.location.hostname+':'+window.location.port+':8080/api/path',
	...
	$scope.api.cancel = function() {
		...
		$http({
			method:'POST',
			url: 'http://'+window.location.hostname+':'+window.location.port+':8080/api/install',
			data: { action: "cancel" }
		});
